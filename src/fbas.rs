...
use std::{
    ...
    fmt::Debug,fs::File, io::Read,rc::Rc
};
...
impl Fbas {
    ...
    fn from_quorum_set_map(qsm: QuorumSetMap) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for (node_str, qset) in qsm.iter() {
            ...
            let q_idx = fbas.process_scp_quorum_set(qset, 0, &known_validators, &mut known_qsets)?;
    ...
    fn process_scp_quorum_set(
        ...
        for inner_qset in &qset.inner_sets {
            let qidx = self.process_scp_quorum_set(inner_qset, curr_depth + 1, known_validators, known_qsets)?;
...
fn try_parse_quorum_set_map_from_json_regular(root: Object) -> Result<QuorumSetMap, Box<dyn std::error::Error>> {
...
fn parse_internal_quorum_set(json_qset: &JsonValue) -> Result<InternalScpQuorumSet, Box<dyn std::error::Error>> {
    ...
    for item in v {
        match item {
            ...
            _ => return Err("validator entry must be either a string (PublicKey) or an object (QuorumSet)".into()),
        }
    }
...
fn parse_stellarbeats_internal_quorum_set(json_qset: &JsonValue) -> Result<InternalScpQuorumSet, Box<dyn std::error::Error>> {
...
fn try_parse_quorum_set_map_from_stellarbeats_json(nodes: Vec<JsonValue>) -> Result<QuorumSetMap, Box<dyn std::error::Error>> {
    let mut quorum_map = QuorumSetMap::new();
    for node in nodes {
        let node = match node {
            JsonValue::Object(n) => n,
        ...
        let public_key = node.get("publicKey")
            .and_then(|n| n.as_str())
...
mod test {
    ...
    fn test_parse_quorum_set_map_from_json() {
        ...
        let test_key = StrKeyPublicKey::from_str("GARHWC6Y4WNGLKCAC7SCFFLEV5GKTKB2AHVIA6C7SU5WLJTDW5W3MPHX").unwrap();
    ...
    fn test_parse_quorum_set_map_from_stellarbeats_json() {
        ...
        // Test parsing of a specific node's quorum set
        let test_key = StrKeyPublicKey::from_str("GD6SZQV3WEJUH352NTVLKEV2JM2RH266VPEM7EH5QLLI7ZZAALMLNUVN").unwrap();
        let test_node_id = test_key.to_string();
